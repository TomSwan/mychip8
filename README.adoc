...
The first command `cd` navigates to the home folder -- or, you can change to any other folder where you want to clone or unzip the repository. In the configuration command (third line from the last), edit the folder names and version numbers as needed to locate qmake on your system (installed along with Qt). If MyTank runs, MyChip8 is working! Press W S A D to navigate, Esc to quit. See <<links>> if you have trouble.

._MyTank Chip8 example_
image::clip-mytank1.gif[640, 400]

To run Breakout, pictured above, try:

[source, console]
....
./mychip8 ../examples/br8kout/br8kout.ch8 
....
