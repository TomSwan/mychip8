...
+++
<a href="https://www.patreon.com/bePatron?u=28168579" data-patreon-widget-type="become-patron-button">Become a Patron!</a><script async src="https://c6.patreon.com/becomePatronButton.bundle.js"></script>
+++
...
Version 0.1 of MyChip8 as described in this manual faithfully executes Cosmac VIP programs outputting to a fixed 32-by-64-pixel, monochrome display, with Chip8 bytecode files loaded into a 4096-byte virtual memory core. Additional features include the following (also see <<options>>):
...
Except for the original interpreter, which I rewrote and debugged from top to bottom, I wrote all of MyChip8 from scratch in C++ using the Qt development system running on Ubuntu Linux, compiled using QMake and the Gnu Compiler Collection (GCC). Any recent release of Qt with OpenGL support should be able to build MyChip8 as fully explained in the next sections. (See <<linksand>> for links to required software.)

image::cosmac-vip.png[]

NOTE: MyChip8 implements all but one of the 35 original Chip8 instructions as programmed for the RCA Cosmac VIP computer (circa 1970s, see photo). MLS, Machine Language CDP1802 Subroutine calls, are not recognized. Missing also is support for SuperChip, Octo and other Chip8 extensions and anomalies -- all of which are priorities, however, on the _must-do_ list of coming MyChip8 attractions.
...
NOTE: Please report bugs and suggest features. Thanks!
...
and then select Configure Project. You only have to perform this step once. After that's done, optionally browse the source code files in the Editor, and then before running, configure the IDE for terminal-based input and output:
...
TIP: Use the -k option to add arrow keys to the usual W A S D navigation hex pad keys.
...
Maybe I'm old fashioned, but I prefer to run MyChip8 from a command-line prompt with the name of a Chip8 file to load and run. That way, I can easily select among several available runtime options. 
...
TIP: Similar commands should work on OS/X (Mac) and Windows systems in a console window. MS Windows batch files are similarly useful. (Pssst: to save even more typing, keep soft links and batch file names really short -- `c8,` for example, is what I actually use.)
