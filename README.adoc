...
:keywords: GAMES VINTAGE PROGRAMMING CHIP8 OCTO C++ RCA COSMAC VIP QT
:copyright: © 2026 by Tom Swan
...
Welcome to _MyChip8 for VIPs,_ my home-brew Cosmac VIP emulator based on a like-named open-source Chip8 interpreter by Laurence Muller (see <<original>>). I wrote, revised and tested MyChip8 with dozens of published Chip8 and Octo games and other programs. MyChip8 runs great, it's all open-source, it's all here, and it's all free.

._Br8kout_
image::clip-br8kout.gif[800, 500]

NOTE: See especially https://github.com/JohnEarnest/chip8Archive and also search for Chip8 for other great examples of Chip8 programming 
...
MyChip8 implements all but one of the 35 original Chip8 instructions as programmed for the Cosmac VIP. MLS (CDP1802 Machine Language Subroutine) calls, are not recognized. SuperChip, Octo and other Chip8 extensions also are unimplemented, but are top priorities. Check back often for updates!
...
I use the second method in Linux, so that's what I describe here. (You can additionally build in the Qt IDE and then run the `mychip8` executable file from the build folder that Qt creates.) Similar commands ought to work in Windows and on Macintosh systems. 

Following are the commands that I use to clone the MyChip8 repository, build the program, and run a sample Chip8 program (`{tilde}` stands for "home folder"):
...
mkdir build
cd build
qmake -o Makefile ../mychip8.pro -spec linux-g++
make
./mychip8 ../examples/mytank.ch8
....

The first command `cd` navigates to your home folder -- or go to any other folder where you want to clone or unzip the repository. Do that using git as shown, or download and unpack the ZIP file if you prefer. Either way, change into the mychip8 folder. Make a `build` or other folder, `cd` into it, and run `qmake` (installed along with Qt) to create a Makefile using the information in mychip8.pro (for 'Qt project file'). If using a different operating system, you probably need to edit the `-spec linux g++` option to select a compiler. After creating Makefile, enter `make` to build the emulator. 

Test MyChip8 it on any .ch8 bytecode file as shown on the last line. If MyTank runs, MyChip8 is working! Press keys W S A D to navigate, Esc to quit. See <<links>> if you have trouble.

._MyTank Chip8 example_
image::clip-mytank1.gif[640, 400]

Try also running the Breakout example depicted at the top of this document:

[source, console]
....
./mychip8 ../examples/br8kout.ch8
....

TIP: Qt Creator can help determine the correct command line options to specify. Start a new, empty Qt console project, build it, and then open the resulting Makefile in the project's build folder. Look for a comment documenting the proper configuration settings for console applications to use on your system for building from the command line using make.
...
----
Command line arguments: examples/mytank.ch8
----
...
NOTE: Complete file is in `examples/myChip8-bin-src.zip`
